/* ===========================
   script.js — rebuilt & fixed
   Works with the HTML you provided
   - smooth parallax (desktop + mobile)
   - soft blurry snow (canvas)
   - social grid (white simpleicons)
   - brand glow (Roblox blue override)
...
(() => {
  ...
  const BG_BASE_SCALE = 1.12;        // match CSS scale so edges never show
  const PARALLAX_STRENGTH = 10;     // amplitude for overlays
  const SNOW_DENSITY_DIV = 140000;  // bigger => fewer flakes
  const MIN_SNOW = 30;

  /* ---------- Social list (from you) ---------- */
  const SOCIALS = [
    ...
    { key:'roblox',    name:'Roblox',    url:['https://roblox.com/users/5279565619/profile','https://www.roblox.com/users/8808804903/profile'], icon:'roblox', color:'#00A2FF' }, // forced blue
    ...
    { key:'github',    name:'GitHub',    url:'https://tiktok.com/@pruuingoo', icon:'github', color:'#181717' }, // left as you provided
  ...
  (function parallaxInit(){
    ...
    if(grad) overlayElems.push(grad);
    if(snowCanvas) overlayElems.push(snowCanvas);

    if(overlayElems.length === 0) return;

    // Use rAF smoothing — set targets from pointer/device, animate in tick()
    let tx = 0, ty = 0;
    let curX = 0, curY = 0;
    const ease = 0.12;
    ...
    // pointermove (desktop and mobile pointer)
    window.addEventListener('pointermove', (e) => {
      setTargetFromPointer(e.clientX, e.clientY);
    }, { passive: true });

    // touch move fallback (some browsers)
    window.addEventListener('touchmove', (e) => {
      if(e.touches && e.touches[0]) setTargetFromPointer(e.touches[0].clientX, e.touches[0].clientY);
    }, { passive: true });

    // deviceorientation fallback (mobile tilt)
    window.addEventListener('deviceorientation', (ev) => {
      if(ev.gamma === null || ev.beta === null) return;
      // gamma [-90,90] left/right, beta [-180,180] front/back
      const gx = Math.max(-30, Math.min(30, ev.gamma)) / 30; // normalize -1..1
      const gy = Math.max(-30, Math.min(30, ev.beta - 30)) / 30; // center a bit
    ...
    function tick(){
      ...
      overlayElems.forEach((el, i) => {
        const depth = (i + 1) * 8;
        const x = (curX / depth).toFixed(3);
        const y = (curY / depth).toFixed(3);
        // use translate3d only; preserve CSS scale
  ...
  (function snowInit(){
    ...
    let flakes = [];
    const base = SNOW_DENSITY_DIV;

    function rebuild(){
      W = canvas.width = window.innerWidth;
      H = canvas.height = window.innerHeight;
      const count = Math.max(MIN_SNOW, Math.round((W*H)/base));
      flakes = [];
  ...
  /* ---------- Build socials grid + per-icon glow animation ---------- */
  (function buildSocialGrid(){
    ...
    SOCIALS.forEach(s => {
      ...
      // create per-icon keyframes dynamically (pulse glow)
...
@keyframes ${key}{
  ...
  100% { filter: drop-shadow(0 0 8px ${rgba}); transform: translateZ(0) scale(1); }
      ...
      // append rule
      try { dynStyle.sheet.insertRule(kfText, dynStyle.sheet.cssRules.length); } catch(e){
        // fallback append text node
        dynStyle.appendChild(document.createTextNode(kfText));
      }
      // set initial styles
      ...
      // click: open main modal or choice modal
      btn.addEventListener('click', (ev) => {
        ev.stopPropagation();
        if(Array.isArray(s.url)){
          // opens choice modal (primary/secondary)
  ...
  (function choiceModal(){
    ...
    function openChoice(title, urls, iconSlug){
      ...
      primaryBtn.className = 'btn btn-open';
      primaryBtn.type = 'button';
      primaryBtn.innerText = 'Primary';
      ...
      secondaryBtn.className = 'btn btn-copy';
      secondaryBtn.type = 'button';
      secondaryBtn.innerText = 'Secondary';
      secondaryBtn.addEventListener('click', ()=>{
        const url = urls[1] || urls[0];
        window.openModal(`${title} — Secondary`, url, iconSlug);
        backdrop.classList.add('hidden');
      });

      buttonsEl.appendChild(primaryBtn);
      buttonsEl.appendChild(secondaryBtn);
    }
